...
def make_emissions(crf, seq_length=3, batch_size=2):
    em = mindspore.Tensor(np.random.randn(seq_length, batch_size, crf.num_tags), mindspore.float32)
    if crf.batch_first:
        em = em.swapaxes(0, 1)
    return em
...
class TestForward:
    ...
    def test_batched_loss(self):
        ...
        assert llh.asnumpy() == approx(total_llh.asnumpy())
    ...
    def test_reduction_none(self):
        ...
        for llh_, manual_llh_ in zip(llh, manual_llh):
            assert llh_.asnumpy() == approx(manual_llh_)
    ...
    def test_reduction_mean(self):
        ...
        assert llh.asnumpy() == approx(manual_llh / batch_size)

    def test_reduction_token_mean(self):
        crf = make_crf(reduction='token_mean')
        seq_length, batch_size = 3, 2
        ...
        assert np.allclose(llh.asnumpy(), (manual_llh / n_tokens).asnumpy(), 1e-3, 1e-3)
